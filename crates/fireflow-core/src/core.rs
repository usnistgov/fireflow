    ...
    M::Name: Clone,
{
    ...
    where
        ...
        M::Temporal: VersionedTemporal<Warning = NeverValue<()>, Error = Infallible>,
    {
        let x = self
            .measurements
        ...
        M: HasUnstainedCenters,
    {
        ...
        LogResult::<_, _, _, _, AlwaysValue<_>, Vec<_>>::new_err_from_iter(it, ())
            .when_ok(|| {
                *self
                    .metaroot
                    .specific
    ...
    <M::Ver as Versioned>::Layout: VersionedDataLayout,
{
        ...
        M::Optical: AsScaleTransform,
    {
        ...
        LogResult::new_non_fungible((), (), e.into(), meas_n != data_n)
            .and_cmt(|| {
                self.set_measurements_inner(xs, allow_shared_names, skip_index_check)
...
impl ConvertFromOptical<InnerOptical3_1> for InnerOptical2_0 {
    fn convert_from_optical(
        ...
        let check_res = [cal, dpy]
            .into_iter()
            .map(LogResult::into_semigroup::<_, Vec<_>, Vec<_>>)
            .mappend_def_void()
...
impl ConvertFromMetaroot<InnerMetaroot2_0> for InnerMetaroot3_2 {
    fn convert_from_metaroot(
        ...
        let mode_res = Mode3_2::try_from(value.mode)
            .into_deferred_fungible_opt::<AlwaysValue<_>, Vec<_>>(!allow_loss)
            .map_cmt_warnings(MetarootConvertWarning::from)
            .map_non_fung_errors(MetarootConvertError::from);
        ...
        match self {
            Self::Lin(x) => {
                if x.is_one() {
                    ...
                    LogResult::new_deferred_fungible(Scale::Linear, e, !allow_loss)
                } else {
    ...
    fn lookup_specific(
        ...
        let process_mode = |mode| {
            ...
            let res = if let Some(e) = err {
                ...
                LogResult::<_, _, _, _, AlwaysValue<_>, Vec<_>>::new_ok(mode)
            };
