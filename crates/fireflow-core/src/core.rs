...
pub trait Versioned {
        ...
        C: AsRef<ReadLayoutConfig> + AsRef<ReaderConfig> + AsRef<ReadTEXTOffsetsConfig>,
    {
        let layout_res = Self::Layout::lookup_ro(kws, st.conf.as_ref())
            ...
            .map_non_fung_errors(LookupAndReadDataAnalysisError::from);
...
pub trait VersionedMetaroot: Sized {
    ...
    > {
        ...
        let t_res = o
            ...
            .map_non_fung_errors(SwapOpticalTemporalError::from)
            .into_semigroup();
        let o_specific_res = t
            .specific
        ...
        let o_common_res = o
            ...
            .map_non_fung_errors(OpticalToTemporalError::from)
            .map_non_fung_errors(SwapOpticalTemporalError::from);
            ...
            .recover_with(
                |(_, (o, d)), es| {
                    if allow_loss {
                        ...
                        Result::new_ok(v).set_cmt_warnings(ws)
                    } else {
                        Result::new_err(es).set_err_value(Box::new(o))
                    }
                },
                |(_, (o, d))| Result::new_ok(Self::from_optical_unchecked(o, d)),
            )
    }
...
impl<T> Temporal<T> {
    ...
    fn convert<ToT>(self, i: MeasIndex, allow_loss: bool) -> TemporalConvertResult<Temporal<ToT>>
    where
...
impl<O> Optical<O> {
    fn try_convert<Of: ConvertFromOptical<O>>(
        ...
        Of::convert_from_optical(self.specific, i, allow_loss).map_ok_value(|specific| {
            Optical::new(
                self.common,
                self.filter,
                self.power,
    ...
    M::Name: Clone,
{
    ...
    pub fn set_temporal(
        ...
        allow_loss: bool,
    ) -> CmtFungibleErrorsResult<bool, (), SetTemporalError>
    where
        ...
        >,
    {
        // TODO ditto above
        self.measurements
            .unset_center(|i, old_t| M::Optical::from_temporal(old_t, i, allow_loss))
    }
    ...
    pub fn set_unstained_centers(
        &mut self,
        us: UnstainedCenters,
    ) -> ErrorsResult<(), (), MissingMeasurementNameError>
    where
        M: HasUnstainedCenters,
    {
        ...
        Result::new_err_from_iter(it, ()).map_ok_value(|()| {
            *self
                .metaroot
                .specific
    ...
    /// Set $PnE (2.0)
    pub fn set_scales(&mut self, scales: Vec<Option<Scale>>) -> ErrorsResult<(), (), SetScalesError>
    where
        M::Optical: HasScale,
    {
        ...
        l.check_transforms_and_len(&xforms[..])
            .map_non_fung_errors(SetScalesError::from)
            .eval_def_error(|()| center_scale_not_linear())?;
    ...
    ) -> ErrorsResult<(), (), SetTransformsError>
    where
        M::Optical: HasScaleTransform,
    {
        let center_xform_not_noop = || {
            self.measurements
                ...
                .is_some_and(|s| s.is_noop())
                .then_some(NonLinearTemporalTransformError.into())
        };

        let l = &self.layout;
        l.check_transforms_and_len(&xforms[..])
            .map_non_fung_errors(SetTransformsError::from)
            .eval_def_error(|_| center_xform_not_noop())?;
            ...
            TryFrom<<M::Name as MightHave>::Wrapper<Shortname>>,
    {
        let root_res = self
            ...
            .map_non_fung_errors(ConvertErrorInner::Meta);
        let meas_res = self
            .measurements
            .map_center_value(|v| v.value.convert(v.index, allow_loss))
            .set_err_value(())
            .map_non_fung_errors(ConvertErrorInner::Temporal)
            .map_cmt_warnings(MetarootConvertWarning::from)
            .and_then_cmt(|meas| {
                meas.map_non_center_values(|i, v| v.try_convert(i, allow_loss))
                    .map_non_fung_errors(ConvertErrorInner::Optical)
                    .map_cmt_warnings(MetarootConvertWarning::from)
            })
            .and_then_cmt(|meas| {
                meas.try_rewrapped()
                    .map_non_fung_errors(ConvertErrorInner::Rewrap)
                    .nowarn_into_warn()
            });
        let layout_res = ConvertFromLayout::convert_from_layout(self.layout)
            .map_non_fung_errors(ConvertErrorInner::Layout)
            .nowarn_into_warn();
        root_res
    ...
    fn push_temporal_inner(
        ...
        r: Range,
        disallow_trunc: bool,
    ) -> DeferredWarningsAndErrors<(), AnyRangeError, InsertTemporalError> {
        self.measurements
            .push_center(n, m)
            ...
            .and_then_def(|()| {
                self.layout
                    ...
                    .map_non_fung_errors(InsertTemporalError::from)
            })
            .map_def_value(|()| {
                self.metaroot
                    .specific
    ...
    fn insert_temporal_inner(
        ...
        self.measurements
            ...
            .map_def_value(|()| self.metaroot.specific.insert_meas_index_inner(i))
    }
    ...
    {
        // TODO aggregate errors eventually
    ...
    ) -> ErrorsResult<(), (), MeasLayoutMismatchError>
    where
        M::Optical: AsScaleTransform,
    {
        layout
        ...
        Version: From<M::Ver>,
    {
        ...
        let ns_res = conf.nonstandard_measurement_pattern.as_ref().map_or(
            Result::new_ok(blank_meas_nonstd()),
            |pat| {
                (0..par.0)
                    .map(|n| {
                    ...
                    .map_err(LookupMeasWarning::from)
    ...
    pub fn try_new_2_0(
        ...
        Timestamps::try_new(btim, etim, date)
            ...
            .and_then_cmt(|ts| {
                ...
                Self::try_new(metaroot, measurements, layout).non_fung_errors_into()
            })
    }
}
...
impl UnstainedData {
    ...
    fn check_loss(self, allow_loss: bool) -> DeferredFungibleErrors<(), AnyMetarootKeyLossError> {
        ...
        [c, i].into_iter().map(|r| r.repack()).mappend_def_void()
    }
}
...
impl CSVFlags {
                ...
                if let Some(n) = m {
                    ...
                    fs.mappend_def().and_then_def(|flags| {
                        if flags.is_empty() {
                            ...
                            Result::new_deferred_fungible(flags, e, is_err)
                        } else {
                            Result::new_ok(flags)
                        }
                    })
    ...
    fn check_loss(self, allow_loss: bool) -> DeferredFungibleErrors<(), AnyMetarootKeyLossError> {
        self.0
            ...
            .enumerate()
            .map(|(i, f)| {
                f.check_indexed_key_transfer_fungible(i, allow_loss)
                    .repack()
            })
            .mappend_def()
            .set_def_value(())
            .eval_def_fung_error(!allow_loss, |()| {
                (!self.0.is_empty()).then_some(UnitaryKeyLossError::<CSMode>::new().into())
            })
    }
}
...
impl ModificationData {
    ...
    fn check_loss(self, allow_loss: bool) -> DeferredFungibleErrors<(), AnyMetarootKeyLossError> {
        ...
        [d, r, o].into_iter().map(|r| r.repack()).mappend_def_void()
    }
}
...
impl CarrierData {
    ...
    fn check_loss(self, allow_loss: bool) -> DeferredFungibleErrors<(), AnyMetarootKeyLossError> {
        ...
        [i, t, l].into_iter().map(|r| r.repack()).mappend_def_void()
    }
}
...
impl PlateData {
    ...
    fn check_loss(self, allow_loss: bool) -> DeferredFungibleErrors<(), AnyMetarootKeyLossError> {
        ...
        [n, i, w].into_iter().map(|r| r.repack()).mappend_def_void()
    }
}
...
impl PeakData {
    ...
    fn check_loss(
        ...
        let b = self.bin.check_indexed_key_transfer_fungible(i, allow_loss);
        let s = self.size.check_indexed_key_transfer_fungible(i, allow_loss);
        [b, s].into_iter().map(|r| r.repack()).mappend_def_void()
    }
}
...
impl ConvertFromOptical<InnerOptical3_2> for InnerOptical2_0 {
    fn convert_from_optical(
        ...
        let meas = value
            .measurement_type
            .check_indexed_key_transfer_fungible(i, allow_loss);
        let tag = value.tag.check_indexed_key_transfer_fungible(i, allow_loss);
        let det_name = value
            .detector_name
            .check_indexed_key_transfer_fungible(i, allow_loss);
...
impl ConvertFromOptical<InnerOptical3_0> for InnerOptical3_1 {
    fn convert_from_optical(
        ...
        let wave = value.wavelength.map(Wavelengths::from).unwrap_or_default();
        Result::new_ok(Self::new(value.scale, wave, None, None, value.peak))
    }
}
...
impl ConvertFromMetaroot<InnerMetaroot3_1> for InnerMetaroot3_0 {
    fn convert_from_metaroot(
        ...
        let plate = value.plate.check_loss(allow_loss);
        let modi = value.modification.check_loss(allow_loss);
        let vol = value.vol.check_key_transfer(allow_loss).into_semigroup();
        [plate, modi, vol]
            .into_iter()
            .mappend_def_void()
            .map_cmt_warnings(MetarootConvertWarning::from)
...
impl ConvertFromMetaroot<InnerMetaroot2_0> for InnerMetaroot3_2 {
    fn convert_from_metaroot(
        ...
        let check_res = Result::new_ok(())
            .eval_def_non_fung_error(!allow_loss, |_| {
                (!value.applied_gates.is_empty()).then_some(gating::AppliedGates2_0To3_2Error)
            })
            .eval_def_non_fung_error(!allow_loss, |_| {
                (!value.comp.is_none()).then_some(Comp2_0TransferError)
            });
        ...
        match self {
            Self::Lin(x) => {
                if x.is_one() {
                    ...
                    Result::new_deferred_fungible(Scale::Linear, e, !allow_loss)
                } else {
                    Result::new_ok(Scale::Linear)
                }
            }
            Self::Log(x) => Result::new_ok(Scale::Log(x)),
        }
    }
...
impl ConvertFromTemporal<InnerTemporal2_0> for InnerTemporal3_0 {
    fn convert_from_temporal(
        ...
        Result::new_ok(Self::new(Timestep::default(), value.peak))
    }
}
...
impl ConvertFromTemporal<InnerTemporal2_0> for InnerTemporal3_1 {
    fn convert_from_temporal(
        ...
        Result::new_ok(Self::new(Timestep::default(), None, value.peak))
    }
}
...
impl ConvertFromTemporal<InnerTemporal3_0> for InnerTemporal3_1 {
    fn convert_from_temporal(
        ...
        Result::new_ok(Self::new(value.timestep, None, value.peak))
    }
}
...
impl ConvertFromLayout<DataLayout3_0> for DataLayout2_0 {
    fn convert_from_layout(value: DataLayout3_0) -> LayoutConvertResult<Self> {
        Result::new_ok(Self(value.0.phantom_into()))
    }
}
...
impl ConvertFromLayout<DataLayout3_1> for DataLayout3_2 {
    fn convert_from_layout(value: DataLayout3_1) -> LayoutConvertResult<Self> {
        Result::new_ok(Self::NonMixed(value.0.phantom_into()))
    }
}
...
impl VersionedTemporal for InnerTemporal3_2 {
    ...
    fn can_convert_to_optical(&self, i: MeasIndex) -> DeferredError<(), Self::Error> {
        self.measurement_type
            .check_indexed_key_transfer(i)
            .map_non_fung_errors(TemporalToOpticalError::Loss)
    }
...
impl OpticalFromTemporal<InnerTemporal2_0> for InnerOptical2_0 {
    ...
    ) -> ErrorsResult<(Optical<Self>, Self::TData), Box<Temporal<InnerTemporal2_0>>, Infallible>
    {
        t.specific
            ...
            .from_infallible()
            .set_ok_value(Self::from_temporal_unchecked(t))
    }
            ...
            .recover_with(
                |(_, t), es| {
                    if allow_loss {
                        ...
                        Result::new_err(es).set_err_value(Box::new(t))
                    }
                },
                |(_, t)| Result::new_ok(Self::from_temporal_unchecked(t)),
            )
    }
    ...
    fn lookup_specific(
        ...
        let process_mode = |mode| {
            ...
            res.map_cmt_warnings(LookupKeysWarning::from)
                .map_non_fung_errors(LookupKeysWarning::from)
                .map_non_fung_errors(LookupKeysError::from)
        };
