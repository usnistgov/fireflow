...
#[cfg(all(feature = "dash_args", feature = "underscore_args"))]
compile_error!("dash and underscore args cannot be set at one time");

#[cfg(feature = "dash_args")]
macro_rules! string_arg_inner {
    ($field:ident) => {{ str_replace!(stringify!($field), "_", "-") }};
}

#[cfg(feature = "underscore_args")]
macro_rules! string_arg_inner {
...
pub const TRUNCATE_OFFSET_LIMIT: &str = offset_arg!(dataset_overflow_limit);
...
pub const IGNORE_TIME_OPTICAL_KEYS: &str = std_kw_arg!(ignore_optical_only_keys);
pub const PROCESS_TIME_OPTICAL_KEYS: &str = std_kw_arg!(process_optical_only_keys);
...
pub const APPEND_STD_KEY_VALS: &str = data_kw_arg!(append_standard_keywords);
...
pub const ALLOW_HEADER_TEXT_OFFSET_MISMATCH: &str = data_kw_arg!(allow_header_text_offset_mismatch);
pub const ALLOW_MISSING_REQUIRED_OFFSETS: &str = data_kw_arg!(allow_missing_required_offsets);
