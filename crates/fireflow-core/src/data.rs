                        ...
                        let new = match d0 {
                            AlphaNumType::Ascii => {
                                let new = x
                                    ...
                                    .collect();
                                NonMixedEndianLayout::from(AnyAsciiLayout::Fixed(Fixed::new(
                                    new, NoByteOrd,
                                )))
                            }
                            AlphaNumType::Integer => {
                                let new = x
                                    ...
                                    .collect();
                                let mut l = NonMixedEndianLayout::Uint(AnyEndianUint::Multi(
                                    Fixed::new(new, x.byte_layout),
                                ));
                                l.normalize();
                                l
                            }
                            AlphaNumType::Float => {
                                let new = x
                                    ...
                                    .collect();
                                NonMixedEndianLayout::F32(Fixed::new(new, x.byte_layout))
                            }
                            AlphaNumType::Double => {
                                let new = x
                                    .columns
                                    ...
                                    .collect();
                                NonMixedEndianLayout::F64(Fixed::new(new, x.byte_layout))
                            }
                        };
                        Self::NonMixed(new)
                    } else {
...
impl<D> AnyEndianUintLayout<D> {
    ...
    where
        D: IsNumType,
    {
        FixedLayout::try_new(cs, e, |i, c| {
            AnyUint::from_width_and_range(c.width, c.range, i, flag).repack_errors()
        })
        .map_ok_value(|res| {
            res.fmap_once(|c| {
                let wrapped = Self::Multi(c);
                // TODO normalize
                wrapped
            })
        })
...
impl<C, S, T, D> LayoutRanges for FixedLayout<C, S, T, D>
where
...
impl<C, S, T, D> LayoutDatatype for FixedLayout<C, S, T, D>
where
    C: HasDatatype,
{
    fn datatype(&self) -> AlphaNumType {
...
impl<C, S, T, D> LayoutKeywords for FixedLayout<C, S, T, D>
where
...
impl HasDatatype for MixedRange {
    fn datatype(&self) -> AlphaNumType {
        match self {
            Self::Ascii(_) => AlphaNumType::Ascii,
            Self::Uint(_) => AlphaNumType::Integer,
...
impl<D> NonMixedEndianLayout<D> {
    ...
    pub fn new_uint(columns: Vec<AnyNullBitmask>, endian: Endian) -> Self {
        AnyEndianUint::Multi(FixedLayout::new(columns, endian)).into()
    }
    ...
    pub(crate) fn try_nonmixed_into_ordered<T>(self) -> LayoutConvertResult<AnyOrderedLayout<T>> {
        // TODO normalize
        match self {
